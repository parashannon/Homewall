...
while current_date <= end_date:
    ...
    command = f"python camping2.py --start-date {(current_date + timedelta(days=0)).strftime('%Y-%m-%d')} --end-date \
    {(current_date + timedelta(days=1)).strftime('%Y-%m-%d')} --show-campsite-info --parks "+ siteid+ " > output.txt"
