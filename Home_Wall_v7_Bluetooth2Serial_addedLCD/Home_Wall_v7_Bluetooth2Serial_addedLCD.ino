...
*/
  ...
  if (command_received) {
    ...
    if (command_in[0] == 'L') {
      ...
      Serial1.println(command);  // if it doesn't start with an L, pass the command off to the other arduino
    }
    //  Serial.flush();
...
void set_LCD_array() {
  ...
  if (command_in[0] == 'L' && command_in[1] == '1') {
    row_index = 0;
  } else if (command_in[0] == 'L' && command_in[1] == '2') {
    row_index = 1;
  } else if (command_in[0] == 'L' && command_in[1] == '3') {
    row_index = 2;
  } else if (command_in[0] == 'L' && command_in[1] == '4') {
    row_index = 3;
  } else if (command_in[0] == 'L' && command_in[1] == 'X') {
    lcd.init();
    delay(100);
    lcd.clear();
    t_last_LCD = millis();
  ...
  if (row_index < 4) {  // only update if the row is good
    for (int i = 0; i < 20; i = i + 1) {

      if (command_in[i + 2] == 13 || command_in[i + 2] == 10) {
        end_found = true;  // that was the end, the rest should be spaces
      }
      if (!end_found) {
        current_char = command_in[i + 2];
      } else {
        current_char = 32;
      }
...
void set_LCD() {
  ...
  if (millis() > t_last_LCD + 1100) {
    Serial.print(line_to_update);
    Serial.println(" :");
    for (int i = 0; i < 20; i = i + 1) {
      lcd_line_next[i] = buff_lcd_line[line_to_update][i];
      Serial.print(lcd_line_next[i]);
    }
    Serial.println("");
    lcd.setCursor(0, line_to_update);
    delay(100);
    lcd.println(lcd_line_next);
...
void GetCommandSerial() {
  ...
  while (Serial1.available() > 0) {
    ...
    command_in[i_buff] = Serial1.read();
    delay(1);
    command_received = true;
    command_from = 1;
